...
func testUserToManyIPAddressUsers(t *testing.T) {
	var err error
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	...
	ipAddressUser, err := a.IPAddressUsers(tx).All()
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadIPAddressUsers(tx, false, &slice); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IPAddressUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IPAddressUsers = nil
	if err = a.L.LoadIPAddressUsers(tx, true, &a); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IPAddressUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpIPAddressUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIPAddressUsers(tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IPAddressUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IPAddressUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IPAddressUsers(tx).Count()
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpIPAddressUsers(t *testing.T) {
	var err error
	...
	err = a.SetIPAddressUsers(tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetIPAddressUsers(tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.IPAddressUsers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.IPAddressUsers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpIPAddressUsers(t *testing.T) {
	var err error
	...
	err = a.AddIPAddressUsers(tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveIPAddressUsers(tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.IPAddressUsers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.IPAddressUsers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.IPAddressUsers[0] != &e {
...
var (
	userDBTypes = map[string]string{`LastConnected`: `timestamp without time zone`, `Name`: `character varying`, `SteamID`: `character varying`, `UserID`: `integer`}
	_           = bytes.MinRead
)
