...
type User struct {
	...
	SteamID       null.String `boil:"steam_id" json:"steam_id,omitempty" toml:"steam_id" yaml:"steam_id,omitempty"`
	Name          null.String `boil:"name" json:"name,omitempty" toml:"name" yaml:"name,omitempty"`
	LastConnected null.Time   `boil:"last_connected" json:"last_connected,omitempty" toml:"last_connected" yaml:"last_connected,omitempty"`
...
var (
	userColumns               = []string{"user_id", "steam_id", "name", "last_connected"}
	userColumnsWithoutDefault = []string{"steam_id", "name", "last_connected"}
...
// IPAddressUsersG retrieves all the ip_address_user's ip address users.
func (o *User) IPAddressUsersG(mods ...qm.QueryMod) ipAddressUserQuery {
	return o.IPAddressUsers(boil.GetDB(), mods...)
}

// IPAddressUsers retrieves all the ip_address_user's ip address users with an executor.
func (o *User) IPAddressUsers(exec boil.Executor, mods ...qm.QueryMod) ipAddressUserQuery {
...
// LoadIPAddressUsers allows an eager lookup of values, cached into the
// loaded structs of the objects.
func (userL) LoadIPAddressUsers(e boil.Executor, singular bool, maybeUser interface{}) error {
	var slice []*User
	var object *User
	...
	query := fmt.Sprintf(
		"select * from \"ip_address_users\" where \"user_id\" in (%s)",
	...
	if singular {
		object.R.IPAddressUsers = resultSlice
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.UserID.Int {
				local.R.IPAddressUsers = append(local.R.IPAddressUsers, foreign)
				break
			}
		}
	}
...
// AddIPAddressUsers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.IPAddressUsers.
// Sets related.R.User appropriately.
func (o *User) AddIPAddressUsers(exec boil.Executor, insert bool, related ...*IPAddressUser) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			IPAddressUsers: related,
		}
	} else {
		o.R.IPAddressUsers = append(o.R.IPAddressUsers, related...)
	}
...
// SetIPAddressUsers removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's IPAddressUsers accordingly.
// Replaces o.R.IPAddressUsers with related.
// Sets related.R.User's IPAddressUsers accordingly.
func (o *User) SetIPAddressUsers(exec boil.Executor, insert bool, related ...*IPAddressUser) error {
	query := "update \"ip_address_users\" set \"user_id\" = null where \"user_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.IPAddressUsers {
		...
		o.R.IPAddressUsers = nil
	}
	return o.AddIPAddressUsers(exec, insert, related...)
}

// RemoveIPAddressUsers relationships from objects passed in.
// Removes related items from R.IPAddressUsers (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveIPAddressUsers(exec boil.Executor, related ...*IPAddressUser) error {
	var err error
	for _, rel := range related {
		rel.UserID.Valid = false
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.IPAddressUsers {
			if rel != ri {
				continue
			}

			ln := len(o.R.IPAddressUsers)
			if ln > 1 && i < ln-1 {
				o.R.IPAddressUsers[i] = o.R.IPAddressUsers[ln-1]
			}
			o.R.IPAddressUsers = o.R.IPAddressUsers[:ln-1]
			break
		}
	}
